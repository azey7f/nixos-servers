...
in {
  ...
  config = mkIf cfg.enable {
    ...
    az.server.rke2.manifests."00-namespaces" = let
      clusterPrefix = "${config.az.cluster.net.prefix}::/${toString config.az.cluster.net.prefixSize}";
                ...
                spec = {
                  ...
                  egress =
                    ...
                    ++ lib.optional ns.networkPolicy.toCluster {
                      action = "Allow";
                      destination.nets = [clusterPrefix];
                    }
                    # toCIDR
                    ...
                    ++ lib.lists.optionals ns.networkPolicy.toWAN [
                      {
                        ...
                        destination.notNets = [clusterPrefix];
                      }
                      {
                        action = "Allow";
